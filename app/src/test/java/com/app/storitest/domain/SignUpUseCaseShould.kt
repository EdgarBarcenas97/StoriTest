...
class SignUpUseCaseShould {
    ...
    fun `Return UserId when signUp is called and signUp repository are success`() = runTest {
        ...
        whenever(authRepository.signUp(userRegister)).thenReturn(flowOf(resultUserId))

        val result = signUpUseCase.signUp(userRegister).lastOrNull()
    ...
    fun `Get SignUpException when signUp is called and signUp in repository is failure`() = runTest {
        ...
        whenever(authRepository.signUp(userRegister)).thenReturn(flowOf(resultSignUpException))
